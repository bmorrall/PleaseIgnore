    ...
    context 'with a Facebook auth hash' do
      ...
      context 'with an existing account' do
        ...
        it 'updates the existing account with the auth hash' do
          ...
          allow(Accounts::Facebook).to receive(:find_by_uid).and_return(existing_account)
          expect(Account.find_for_oauth(auth_hash)).to eq(existing_account)
        end
      end
  ...
  describe '.new_with_auth_hash' do
    context 'with a Facebook auth hash' do
      ...
      it 'builds a new Accounts::Facebook from a auth hash' do
        ...
        expect(account).to be_kind_of(Accounts::Facebook)
        expect(account).to_not be_persisted
      end
      it 'invokes #update_from_auth_hash on the new Account' do
        expect_any_instance_of(Accounts::Facebook).to receive(:update_from_auth_hash)
...
describe Accounts::Facebook do
  ...
  describe 'validations' do
    it { should allow_value('Accounts::Facebook').for(:type) }
  end
  ...
  describe '#account_uid' do
    ...
    it 'falls back to uid when name is nil' do
      subject.name = ''
